...
def perform_action(player, tile, game):
    ...
    if player.position == 0 or player.last_roll > tile.position:
...
def community_chest_action(player, game):
    ...
    if card.action == 'AP':
        ...
        move_to_card_destination(card, player, game)
...
def move_to_card_destination(card, player, game):
    ...
    if card.destination == 'Jail':
        ...
        if player.position <= 5:
            player.position = 5
            tile = game.tile_rents[str(player.position)]
            if tile.owner:
                Transactions.pay_double_rr_rent(player, tile, game)
            else:
                perform_action(player, tile, game)
